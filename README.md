...
自托管、单用户的腾讯云 **COS（对象存储）** 与 **数据万象 Cloud Infinite（CI）** 桌面应用。你自备大模型 API Key 和腾讯云密钥，在原生窗口里和助手对话，由本机 Node 服务执行一小套高质量 COS/CI 操作。
...
会打开 **COS Harness** 窗口（不是让你去 Chrome 里打开一个网址）。首次进入先到 **设置**：
...
开发时窗口标题为 `COS Harness`，默认大小约 1280×840，任务栏 / Dock 使用应用图标。
